...
<div class="container spacer">
  ...
  <%	} 
  		...
  		<div><h2>Tipos de sesiones</h2></div>
  		<div class="col-lg-6">
  		<form action="ServletLogin?opcion=GenerarNuevoTipoSesion" method="post">	 
				<div class="btn-group  btn-group-lg">
					<button type="submit" class="btn btn-success" name="NuevoCanal" value="Nuevo" title="Crear nuevo tipo de sesión">
                		<i class="fa fa-plus-square fa-1x"></i> <i class="fa fa-caret-square-o-right fa-2x"></i>
            		</button>
				</div>
		</form>	
		</div>
   <%
				Iterator<Tipo_Sesion> iterres1 = tipossesiones.iterator();
				if(!iterres1.hasNext()){  %>
						<h3>No existen datos para mostrar </h3>
	<%			}else{	%>
				<div class="col-lg-6">
				<h4>Resultado(s): <%=tipossesiones.size() %> Tipo(s) de sesión(es)</h4>
				</div>
				<hr>
	...
	<%				while(iterres1.hasNext())
					{
					Tipo_Sesion tp = (Tipo_Sesion)iterres1.next();; 
	%>
						<tr>
							<td><h5><i class="fa fa-caret-square-o-right fa-2x"></i></h5></td>
							<td><h5><%=tp.getId_Tipo_Sesion()%></h5></td>
							<td><h5><%=tp.getTipo_Sesion()%></h5></td>
							<form action="ServletTipoSesion?opcion=CambiarTipoSesion" method="post">
									<input type="hidden" value="<%=tp.getId_Tipo_Sesion() %>" name="34_ID_TIPO_SESION">
							<td>	 
								<div class="btn-group  btn-group-lg">
									 <button type="submit" class="btn btn-success" name="ModificarTipoSesion" value="Modificar" >
            				...
            				<td>
            					<div class="btn-group  btn-group-lg">
								     <button type="submit" class="btn btn-danger" name="EliminarTipoSesion" value="Eliminar" onclick="return confirm('¿Estás seguro que deseas eliminar este tipo de sesión? ¡¡Se borrarán todas las sesiones asociadas a este tipo de sesión!!')">
